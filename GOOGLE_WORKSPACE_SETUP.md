...
Drive remains restricted to direct children of the app-owned folder named
`hermes`. Contacts authorization is separate so it can be enabled without
changing existing Gmail, Calendar, or Drive tokens.
...
Existing Gmail, Calendar, and Drive tokens remain valid. Only Contacts requires
a new one-time authorization.
